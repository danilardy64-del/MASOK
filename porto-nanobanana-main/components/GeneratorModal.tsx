...
export const GeneratorModal: React.FC<GeneratorModalProps> = ({ onClose }) => {
  ...
  const handleGenerate = async () => {
    // When using Gemini 3 series models for image generation, mandatory API key selection is required.
    const aistudio = (window as any).aistudio;
    if (aistudio) {
        const hasKey = await aistudio.hasSelectedApiKey();
        if (!hasKey) {
            await aistudio.openSelectKey();
            // Proceed assuming the selection was successful to mitigate the race condition.
        }
    }

    setState(prev => ({ ...prev, isGenerating: true, error: null }));
    try {
      // The service will create a fresh GoogleGenAI instance using the injected process.env.API_KEY.
      ...
      // If requested entity was not found, reset key selection state and prompt again.
      if (errorMessage.includes("Requested entity was not found") && aistudio) {
          await aistudio.openSelectKey();
          errorMessage = "Please ensure you have selected a valid API key from a paid project.";
      }
        ...
        <div className="w-full md:w-1/2 p-6 md:p-8 bg-slate-50 overflow-y-auto border-b-4 md:border-b-0 md:border-r-4 border-black">
          ...
          <div className="space-y-6">
            ...
            <div className="text-[10px] text-slate-500 font-mono mt-2">
                * Requires a paid GCP project. <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" rel="noreferrer" className="underline">Billing Documentation</a>.
            </div>
